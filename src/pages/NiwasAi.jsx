  ...
  return (
    <div className="flex flex-wrap gap-2 mt-4">
      {list.map((s) => (
        ...
        >
          <Sparkles size={13} className={`text-[#1CB46D] ${disabled ? "opacity-30" : "opacity-70 group-hover:opacity-100"}`} />
          {s}
        </button>
      ))}
    </div>
...
export default function NiwasAi() {
  ...
  const handleVoice = () => {
    if (isChatLocked) return; // Block voice input if locked
  ...
  const runSearch = (rawQuery) => {
    if (!rawQuery.trim() || isChatLocked) return; // Prevent searching if locked
    const query = rawQuery.toLowerCase();

    const userMsg = { role: "user", text: rawQuery };

    // Check count before pushing this 3rd message
  ...
  return (
    <div className="flex h-dvh w-full flex-col overflow-hidden bg-gray-50 lg:h-screen lg:flex-row">
      ...
      {/* 🎬 VIDEO POP-UP MODAL (Playing file imported from assets folder) */}
      {showVideoModal && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-[fadeInUp_0.2s_ease-out]">
          <div className="relative w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl transition-all">
            ...
            <div className="relative aspect-video w-full bg-black">
              <video
                src={propertyVideo} // ⭐ Local assets imported video file path here
                controls
                autoPlay
